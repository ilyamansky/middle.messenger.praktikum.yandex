...
export function onSubmit(event: Event, data: FormData) {
  ...
  for (const input of inputs) {
    ...
    if (!isValid) {
      errorCount++;
      //на будущее: добавление класса ошибки к полям формы при сабмите
      /*const elParent = input.closest("div");
      const elHideDiv = elParent?.children[2];
      elHideDiv?.classList.add("show-error-message");
      elHideDiv!.textContent = data[input.name].errorMessage;*/
    } else {
      //на будущее: удаление класса ошибки к полям формы
      /*const elParent = input.closest("div");
      const elHideDiv = elParent?.children[2];
      elHideDiv?.classList.remove("show-error-message");*/
